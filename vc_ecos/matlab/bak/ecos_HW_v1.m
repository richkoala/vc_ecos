...
%     %KKT_factor 误差查看
    ...
    for solve_b_cnt = 1:solve_b_num  
        ...
        for solve_idx = solve_idx_start:solve_idx_end
            ...
            if (CALC_SOLVE_ERR_SHOWEN == 'ON')
                if (solve_idx_start == solve_idx_end)
                    vc_err = MatL*diag(VecD)*MatL'*bx - b;
                    plot(vc_err);title(sprintf('AX=b%01d solve\\_cnt%02d',solve_b_cnt,solve_idx))
                else
                    if (solve_idx==solve_idx_start)
                        vc_err = MatL*diag(VecD)*MatL'*bx - b
                        bx_first = bx;
                        b_first = b;
                        subplot(211);plot(vc_err);title(sprintf('AX=b%01d solve\\_cnt%02d',solve_b_cnt,solve_idx))
                    else
                        vc_err = MatL*diag(VecD)*MatL'*(bx+bx_first) - b_first;
                        subplot(212);plot(vc_err);title(sprintf('AX=b%01d solve\\_cnt%02d',solve_b_cnt,solve_idx))
                    end
                end
            end
