...
#if PROFILING == 3
	,LDL_int* ldl_lsolve2_cnt
	,pfloat* ldl_lsolve2_time
#endif
)

{
	...
	#if PROFILING == 3
	*ldl_lsolve2_time += toc(&t_ldl_lsolve2);
	*ldl_lsolve2_cnt = *ldl_lsolve2_cnt + 1;
	#endif
...
#if PROFILING == 3
	,LDL_int* ldl_dsolve_cnt
	,pfloat* ldl_dsolve_time
#endif
)
{
    LDL_int j ;
	...
	#if PROFILING == 3
	*ldl_dsolve_time += toc(&t_ldl_dsolve);
	*ldl_dsolve_cnt = *ldl_dsolve_cnt + 1;
	#endif
}
...
(
    ...
    double Lx [ ]	/* input of size lnz=Lp[n], not modified */
#if PROFILING == 3
	,LDL_int* ldl_ltsolve_cnt
	,pfloat* ldl_ltsolve_time
#endif
)
{
	LDL_int j, p, p2 ;
	...
	#if PROFILING == 3
		*ldl_ltsolve_time += toc(&t_ldl_ltsolve);
		toc(&t_ldl_ltsolve);
		*ldl_ltsolve_cnt = *ldl_ltsolve_cnt + 1;
	#endif
}
